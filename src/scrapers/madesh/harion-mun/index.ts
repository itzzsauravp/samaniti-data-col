...
export class HarionScraper implements IMunicipalityScraper {
    ...
    async run(config?: ScraperConfig): Promise<void> {
        console.log(`[HarionScraper] Starting ETL run for '${this.municipalityCode}'`);
        ...
        try {
            ...
            console.log(
                `[HarionScraper] Extracted ${pages.length} page(s). Processing incrementally...`,
            );
            ...
            for (const page of pages) {
                try {
                    ...
                    console.error(`[HarionScraper] Error processing page ${page.url}:`, err);
                }
            }
            console.log(`[HarionScraper] ETL run completed successfully.`);
        } catch (err: any) {
            status = "failed";
            errorMsg = err?.message ?? String(err);
            console.error(`[HarionScraper] ETL run failed:`, err);
            throw err;
        } finally {
