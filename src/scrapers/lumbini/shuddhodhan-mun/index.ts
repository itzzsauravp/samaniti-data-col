...
export class ShuddhodhanScraper implements IMunicipalityScraper {
    ...
    async run(config?: ScraperConfig): Promise<void> {
        console.log(`[ShuddhodhanScraper] Starting ETL run for '${this.municipalityCode}'`);
        ...
        try {
            ...
            console.log(
                `[ShuddhodhanScraper] Extracted ${pages.length} page(s). Processing incrementally...`,
            );
            ...
            for (const page of pages) {
                try {
                    ...
                    console.error(`[ShuddhodhanScraper] Error processing page ${page.url}:`, err);
                }
            }
            console.log(`[ShuddhodhanScraper] ETL run completed successfully.`);
        } catch (err: any) {
            status = "failed";
            errorMsg = err?.message ?? String(err);
            console.error(`[ShuddhodhanScraper] ETL run failed:`, err);
            throw err;
        } finally {
